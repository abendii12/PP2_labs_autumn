...
def main():  #в неё упакованы все действия чтобы можно было вызывать main() в конце файла
    ...
    drawing_tool = 'pen'  # 'pen', 'rectangle', 'circle', 'eraser', 'square', 'right_triangle', 'equilateral_triangle', 'rhombus'
    ...
    while True:
        ...
        for event in pygame.event.get(): 
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_p:
                    ...
                    drawing_tool = 'square'  #новая фигура
                elif event.key == pygame.K_4:
                    drawing_tool = 'right_triangle'  #новая фигура
                elif event.key == pygame.K_5:
                    drawing_tool = 'equilateral_triangle'  #новая фигура
                elif event.key == pygame.K_6:
                    drawing_tool = 'rhombus'  #новая фигура
            
            if event.type == pygame.MOUSEBUTTONDOWN:  #событие нажатия любой кнопки мыши
                if event.button == 1:  #левая кнопка
                    if drawing_tool in ['rectangle', 'circle', 'square', 'right_triangle', 'equilateral_triangle', 'rhombus']:
                        start_pos = event.pos  #точка начала фигуры
                    else:
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                if event.button == 1 and start_pos is not None:
                    ...
                    #добавляем новые фигуры в список shapes
                    if drawing_tool in ['rectangle', 'circle', 'square', 'right_triangle', 'equilateral_triangle', 'rhombus']:
                        shapes.append((drawing_tool, start_pos, end_pos, mode))
        ...
        for shape in shapes:
            ...
            if t == 'rectangle':
                ...
                center = start
                radius_shape = int(((end[0] - start[0])**2 + (end[1] - start[1])**2)**0.5)
                pygame.draw.circle(screen, color, center, radius_shape, 3)

            elif t == 'square':  #рисование квадрата
                side = min(abs(end[0] - start[0]), abs(end[1] - start[1]))
                rect = pygame.Rect(start, (side, side))
                pygame.draw.rect(screen, color, rect, 3)

            elif t == 'right_triangle':  #прямоугольный треугольник
                points_tri = [start, (end[0], start[1]), (start[0], end[1])]
                pygame.draw.polygon(screen, color, points_tri, 3)

            elif t == 'equilateral_triangle':  #равносторонний треугольник
                side = abs(end[0] - start[0])
                p1 = (start[0], end[1])
                p2 = (start[0] + side, end[1])
                p3 = (start[0] + side/2, end[1] - side * 0.866)  #высота через sin60
                pygame.draw.polygon(screen, color, [p1, p2, p3], 3)

            elif t == 'rhombus':  #ромб
                cx = (start[0] + end[0]) // 2
                cy = (start[1] + end[1]) // 2
                points_rh = [(cx, start[1]), (end[0], cy), (cx, end[1]), (start[0], cy)]
                pygame.draw.polygon(screen, color, points_rh, 3)
        ...
        if start_pos is not None:
            ...
            elif drawing_tool == 'square':  #предпросмотр квадрата
                side = min(abs(current_pos[0] - start_pos[0]), abs(current_pos[1] - start_pos[1]))
                rect = pygame.Rect(start_pos, (side, side))
                pygame.draw.rect(screen, color, rect, 3)

            elif drawing_tool == 'right_triangle':  #предпросмотр прямоугольного
                points_tri = [start_pos, (current_pos[0], start_pos[1]), (start_pos[0], current_pos[1])]
                pygame.draw.polygon(screen, color, points_tri, 3)

            elif drawing_tool == 'equilateral_triangle':  #предпросмотр равностороннего
                side = abs(current_pos[0] - start_pos[0])
                p1 = (start_pos[0], current_pos[1])
                p2 = (start_pos[0] + side, current_pos[1])
                p3 = (start_pos[0] + side/2, current_pos[1] - side * 0.866)
                pygame.draw.polygon(screen, color, [p1, p2, p3], 3)

            elif drawing_tool == 'rhombus':  #предпросмотр ромба
                cx = (start_pos[0] + current_pos[0]) // 2
                cy = (start_pos[1] + current_pos[1]) // 2
                points_rh = [(cx, start_pos[1]), (current_pos[0], cy), (cx, current_pos[1]), (start_pos[0], cy)]
                pygame.draw.polygon(screen, color, points_rh, 3)
